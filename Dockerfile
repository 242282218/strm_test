...
CMD ["sh", "-c", "if [ ! -f \"$CONFIG_PATH\" ]; then cp /app/config.clawcloud.example.yaml \"$CONFIG_PATH\"; fi && uvicorn app.main:app --host 0.0.0.0 --port ${PORT:-18000} --workers ${WEB_CONCURRENCY:-1}"]

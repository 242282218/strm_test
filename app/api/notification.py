...
class ChannelCreate(BaseModel):
    channel_type: str = Field(..., pattern="^(telegram|serverchan)$")
    channel_name: str
    config: dict
...
def create_channel(channel: ChannelCreate, db: Session = Depends(get_db)):
    ...
    db_channel = NotificationChannel(
        channel_type=channel.channel_type, channel_name=channel.channel_name, config=channel.config
    )
    db.add(db_channel)
    db.commit()
...
async def delete_channel(
    channel_id: int, db: Session = Depends(get_db), service: NotificationService = Depends(get_notification_service)
):
    """删除渠道"""
    db_channel = db.query(NotificationChannel).filter(NotificationChannel.id == channel_id).first()
    if not db_channel:
...
async def create_rule(
    rule: RuleCreate, db: Session = Depends(get_db), service: NotificationService = Depends(get_notification_service)
):
    """创建通知规则"""
    db_rule = NotificationRule(channel_id=rule.channel_id, event_type=rule.event_type, keywords=rule.keywords)
    db.add(db_rule)
    db.commit()
    db.refresh(db_rule)
...
async def delete_rule(
    rule_id: int, db: Session = Depends(get_db), service: NotificationService = Depends(get_notification_service)
):
    """删除规则"""

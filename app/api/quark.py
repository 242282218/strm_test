...
from app.core.constants import MAX_FILES_LIMIT, MAX_PATH_LENGTH
from app.core.dependencies import get_only_video_flag, get_quark_cookie
...
def _get_runtime_quark_config():
    return get_config_service().get_config().quark


def _build_safe_quark_config() -> dict[str, Any]:
    quark_config = _get_runtime_quark_config()
    return {
...
async def _workflow_task_update(task_id: str, **fields: Any) -> dict[str, Any] | None:
...
def _handle_exception(e: Exception, message: str) -> HTTPException:
    """统一异常处理"""
    logger.error(f"{message}: {e!s}")
    return HTTPException(status_code=500, detail=f"{message}: {e!s}")
...
async def get_download_link(file_id: str, cookie: str = None, _cookie: str = Depends(get_quark_cookie)):
    """
    获取下载直链
...
async def get_transcoding_link(file_id: str, cookie: str = None):
    ...
    Returns:
        转码链接信息
    """
    cookie = cookie or _get_runtime_quark_config().cookie

    if not cookie:
        raise HTTPException(
            status_code=400, detail="Cookie is required. Please provide cookie parameter or set it in config.yaml"
        )
...
async def test_link_endpoint():
    ...
    return {
        ...
        "headers": {"Cookie": "test_cookie", "Referer": "https://pan.quark.cn/"},
        "test": True,
    }
...
async def browse_quark_directory(
    ...
    file_type: str | None = Query(None, description="文件类型过滤：video/folder/all"),
    _cookie: str = Depends(get_quark_cookie),
):
    """
    浏览夸克云盘目录
    ...
    try:
        service = QuarkService(cookie=_cookie)
        result = await service.list_files(pdir_fid=pdir_fid, page=page, size=size)
        ...
        raise HTTPException(status_code=500, detail=f"浏览目录失败: {e!s}")
    finally:
        if service:
            await service.close()
...
async def get_quark_config():
    ...
    """
    try:
        return _build_safe_quark_config()
    except InputValidationError:
        raise
    except Exception as e:
...
async def sync_quark_files(
    ...
    """
    quark_config = _get_runtime_quark_config()
    cookie = cookie or quark_config.cookie
    root_id = root_id or quark_config.root_id
    ...
    try:
        ...
        result = await generate_strm_from_quark(
            cookie=cookie, output_dir=output_dir, root_id=root_id, only_video=only_video, max_files=max_files
        )
...
async def test_quark_smart_rename_ai_connectivity(
    ...
    Uses unified AI providers from configuration.
    """
    service = get_ai_connectivity_service()
    results = await service.test_providers(timeout_seconds=timeout_seconds)
    # results 可能是列表或字典，统一转换为列表[ {provider, connected, message} ]
    normalized = []
    if isinstance(results, dict):
        for name, item in results.items():
            if isinstance(item, dict):
                normalized.append({"provider": name, **item})
            else:
                normalized.append({"provider": name, "connected": bool(item), "message": str(item)})
    else:
        for item in results:
            if isinstance(item, dict):
                normalized.append(item)
            else:
                normalized.append({"provider": str(item), "connected": bool(item)})

    return {
        "success": True,
        "interface": "quark_smart_rename",
        "all_connected": all(item.get("connected") for item in normalized),
...
async def smart_rename_cloud_files(request: QuarkSmartRenameRequest, _cookie: str = Depends(get_quark_cookie)):
    """
    智能重命名夸克云盘文件（预览）
    ...
    try:
        ...
        if use_fast_mode:
            ...
            video_files = await quark_service.get_all_video_files(
                pdir_fid=request.pdir_fid, recursive=request.options.get("recursive", True), max_files=1000
            )
            ...
            async def process_file(file_data: dict) -> SmartRenameItem:
                ...
                try:
                    ...
                    parse_error = str(exc)
                    logger.warning(
                        f"Cloud parse failed for fid={fid}, filename={filename}, fallback to standard: {exc}"
                    )
                    parsed_info, used_algorithm, parse_confidence = await rename_service._parse_with_algorithm(
                        filename,
                ...
                try:
                    tmdb_match, match_confidence = await asyncio.wait_for(
                        rename_service._match_tmdb(parsed_info, media_type_hint=parsed_info.get("media_type")),
                ...
                if parse_error and not item.confirmation_reason:
                    item.needs_confirmation = True
                    item.confirmation_reason = "AI 解析失败，已降级为标准解析"
        ...
        for file_data in video_files:
            ...
            # 使用智能重命名服务解析
            parsed_info, used_algorithm, parse_confidence = await rename_service._parse_with_algorithm(
                filename, algorithm, force_ai=request.force_ai_parse
            )

            # TMDB 匹配
            tmdb_match, match_confidence = await rename_service._match_tmdb(
                parsed_info, media_type_hint=parsed_info.get("media_type")
            )
        ...
        raise HTTPException(status_code=500, detail=f"智能重命名预览失败: {e!s}")
    finally:
        if quark_service:
...
async def execute_cloud_rename(request: QuarkRenameExecuteRequest, _cookie: str = Depends(get_quark_cookie)):
    """
    执行云盘文件重命名
    ...
    try:
        ...
        for i in range(0, len(request.operations), batch_size):
            ...
            for op in batch:
                try:
                    # 调用夸克 API 重命名
                    rename_result = await quark_service.rename_file(fid=op.fid, new_name=op.new_name)
                    ...
                    results.append({"fid": op.fid, "status": "failed", "error": str(e)})
                    failed_count += 1
        ...
        raise HTTPException(status_code=500, detail=f"执行重命名失败: {e!s}")
    finally:
        if quark_service:
            await quark_service.close()

...
class SmartRenamePreviewRequest(BaseModel):
    ...
    naming_config: NamingConfigRequest | None = None
...
async def smart_preview(
    ...
    try:
        ...
        # 执行预览
        result = await service.preview(target_path=request.target_path, options=options)
...
async def smart_execute(
    ...
    """
    try:
        result = await service.execute(batch_id=request.batch_id, operations=request.operations)
...
async def validate_filename(
    request: ValidationRequest | None = Body(default=None), filename: str | None = Query(default=None)
) -> ValidationResponse:
    """
    验证文件名是否符合 Emby 命名规范
...
async def get_smart_rename_status():
    ...
    try:
        ...
        return {"available": False, "smart_rename_service": False, "error": str(e)}

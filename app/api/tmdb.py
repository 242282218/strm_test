...
def _get_tmdb_service():
    ...
    return get_tmdb_service(api_key=api_key, language="zh-CN", cache_service=cache_service)
...
async def search_movie(
    ...
    try:
        ...
        raise HTTPException(status_code=500, detail="Failed to search movie")


@router.get("/search/tv", response_model=SearchResultResponse)
async def search_tv(
    query: str = Query(..., description="搜索关键词"),
    first_air_date_year: int | None = Query(None, description="首播年份"),
    page: int = Query(1, ge=1, description="页码"),
):
    """
    搜索电视剧
    ...
    try:
        ...
        raise HTTPException(status_code=500, detail="Failed to search TV")
...
async def get_movie_detail(movie_id: int):
    ...
    try:
        ...
        return MovieDetailResponse(
            ...
            genres=[GenreResponse(id=g.id, name=g.name) for g in detail.genres],
        ...
        raise HTTPException(status_code=500, detail="Failed to get movie detail")
...
async def get_tv_detail(tv_id: int):
    ...
    try:
        ...
        raise HTTPException(status_code=500, detail="Failed to get TV detail")


@router.get("/tv/{tv_id}/season/{season_number}/episode/{episode_number}", response_model=EpisodeResponse)
async def get_episode_detail(tv_id: int, season_number: int, episode_number: int):
    """
    获取剧集详情
    ...
    try:
        tmdb_service = _get_tmdb_service()
        episode = await tmdb_service.get_tv_episode(tv_id, season_number, episode_number)
        ...
        raise HTTPException(status_code=500, detail="Failed to get episode detail")

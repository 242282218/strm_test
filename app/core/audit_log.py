...
class AuditRecord:
    ...
    # 操作详情
    resource_type: str | None  # 资源类型（file/config/task等）
    resource_id: str | None  # 资源ID
    resource_name: str | None  # 资源名称

    # 变更内容
    before_value: Any | None  # 变更前的值
    after_value: Any | None  # 变更后的值
...
class AuditLogger:
    ...
    async def _cleanup_old_files(self):
        """清理旧日志文件"""
        try:
            log_files = sorted(self.log_dir.glob("audit_*.log"), key=lambda p: p.stat().st_mtime, reverse=True)
    ...
    async def log_rename(self, old_name: str, new_name: str, resource_type: str = "file", **kwargs):
        """记录重命名操作"""
        await self.log(
    ...
    async def log_delete(self, resource_name: str, resource_type: str = "file", **kwargs):
        """记录删除操作"""
        await self.log(
    ...
    async def log_config_update(self, config_key: str, old_value: Any, new_value: Any, **kwargs):
        """记录配置变更"""

        # 敏感信息脱敏
        def mask_sensitive(value):
            if isinstance(value, str) and any(
                k in config_key.lower() for k in ["password", "token", "secret", "cookie", "key"]
            ):
                return "***masked***"
            return value
    ...
    async def log_task(self, task_action: AuditAction, task_id: str, task_name: str, status: str = "success", **kwargs):
        """记录任务操作"""
        await self.log(
            action=task_action,
...
def audit_log(
    ...
    level: AuditLevel = AuditLevel.INFO,
    resource_type: str | None = None,
    get_resource_name: Callable[..., Any] | None = None,
):
    """
    审计日志装饰器

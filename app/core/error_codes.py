...
ERROR_MESSAGES: dict[ErrorCode, dict[str, str]] = {
    # 认证错误
    ErrorCode.AUTH_UNAUTHORIZED: {"title": "未授权", "message": "请先登录后再操作", "action": "前往登录"},
    ...
    # 外部服务错误
    ErrorCode.EXTERNAL_TIMEOUT: {"title": "请求超时", "message": "服务器响应时间过长，请稍后重试", "action": "重试"},
...
def get_error_message(code: ErrorCode) -> dict[str, str]:
    ...
    """
    return ERROR_MESSAGES.get(code, {"title": "操作失败", "message": "发生未知错误，请稍后重试", "action": "重试"})

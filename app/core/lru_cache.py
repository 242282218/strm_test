...
class LRUCache:
    ...
    def __init__(self, maxsize: int = 1000, ttl: int | None = None, enable_stats: bool = True):
        """
        初始化LRU缓存
        ...
        # 统计信息
        self._stats = {"hits": 0, "misses": 0, "sets": 0, "deletes": 0, "evictions": 0, "expirations": 0}
...
class CachedFunction:
    ...
    def __init__(self, cache: LRUCache, key_func: Callable | None = None, ttl: int | None = None):
        """
        初始化函数缓存
    ...
    def __call__(self, func: Callable):
        ...
        def wrapper(*args, **kwargs):
            ...
            if self.key_func:
                ...
                key_str = f"{func.__name__}:{args!s}:{sorted(kwargs.items())!s}"
...
def cached(cache: LRUCache, key_func: Callable | None = None, ttl: int | None = None):
    """
    函数缓存装饰器便捷函数
...
class MultiLevelLRUCache:
    ...
    def __init__(
        self, l1_maxsize: int = 100, l2_maxsize: int = 1000, l1_ttl: int | None = 60, l2_ttl: int | None = 3600
    ):
        """
        初始化多级缓存
...
def get_default_cache(maxsize: int = 1000, ttl: int | None = 3600) -> LRUCache:
    """
    获取默认LRU缓存实例

...
class MetricsCollector:
    ...
    def get_recent_points(self, metric_name: str, limit: int = 100) -> list[MetricPoint]:
        """
        获取最近的指标数据点
...
class SystemMonitor:
    ...
    def _collect_system_metrics(self):
        ...
        try:
            ...
            # 同步到 Prometheus
            self._sync_to_prometheus(cpu_percent, memory, disk)

        except Exception as e:
            logger.error(f"Failed to collect system metrics: {e}")

    def _sync_to_prometheus(self, cpu_percent: float, memory, disk):
        """
        同步系统指标到 Prometheus
        ...
        """
        try:
            from app.core.prometheus_metrics import update_system_metrics
            ...
            logger.error(f"Failed to sync metrics to Prometheus: {e}")
...
class AlertManager:
    ...
    def _check_single_threshold(self, threshold: AlertThreshold, current_time: float):
        ...
        # 检查阈值条件
        condition_met = self._evaluate_condition(current_value, threshold.threshold_value, threshold.comparison)
    ...
    def _trigger_alert(self, threshold: AlertThreshold, value: float, timestamp: float):
        ...
        logger.warning(
            f"ALERT TRIGGERED: {threshold.metric_name} {threshold.comparison} {threshold.threshold_value}, current={value}"
        )
    ...
    def _resolve_alert(self, threshold: AlertThreshold, timestamp: float):
        ...
        for alert in reversed(self.alerts):
            if alert["metric_name"] == threshold.metric_name and alert["status"] == "active":
...
def setup_default_monitoring():
    ...
    # 添加默认告警阈值
    alert_manager.add_threshold(AlertThreshold("system.cpu.percent", 80.0, "gt", duration=60.0))
    alert_manager.add_threshold(AlertThreshold("system.memory.percent", 85.0, "gt", duration=120.0))
    alert_manager.add_threshold(AlertThreshold("system.disk.percent", 90.0, "gt", duration=300.0))
    alert_manager.add_threshold(AlertThreshold("scrape.job.failure_rate", 0.3, "gt", duration=120.0))
    alert_manager.add_threshold(AlertThreshold("emby.webhook.event_latency_seconds", 60.0, "gt", duration=120.0))
    alert_manager.add_threshold(AlertThreshold("emby.delete.execute.warning", 0.0, "gt", duration=1.0))

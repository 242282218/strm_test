...
def get_configured_local_directory() -> str | None:
    app_config = get_config_service().get_config()
    endpoints = getattr(app_config, "endpoints", []) or []
    if not endpoints:
        return None

    endpoint = endpoints[0]
    endpoint_dirs = endpoint.get("dirs", []) if isinstance(endpoint, dict) else getattr(endpoint, "dirs", [])
...
def get_allowed_directories() -> list[str]:
    ...
    # 尝试从配置读取
    try:
        local_dir = get_configured_local_directory()
        if local_dir:
...
def validate_file_path(
    file_path: str, allowed_dirs: list[str] | None = None, check_exists: bool = False, allow_symlinks: bool = False
) -> str:
    """
    验证文件路径是否在允许的目录范围内
    ...
    if not in_allowed:
        logger.warning(f"Path security violation: {real_path} is not in allowed directories: {allowed_dirs}")
        raise PathSecurityError("Access denied: file path is outside of allowed directories")
...
def safe_open(
    file_path: str, mode: str = "r", encoding: str | None = None, allowed_dirs: list[str] | None = None, **kwargs
):
    """
    安全地打开文件
...
def safe_makedirs(path: str, mode: int = 0o755, exist_ok: bool = True, allowed_dirs: list[str] | None = None) -> str:
    """
    安全地创建目录
...
def safe_rename(src: str, dst: str, allowed_dirs: list[str] | None = None) -> None:
    """
    安全地重命名/移动文件
...
def safe_symlink(src: str, dst: str, allowed_dirs: list[str] | None = None) -> None:
    """
    安全地创建符号链接
...
def safe_hardlink(src: str, dst: str, allowed_dirs: list[str] | None = None) -> None:
    """
    安全地创建硬链接

        ...
        async with self._lock:
            try:
                async with (
                    ...
                    db.execute(
                        "SELECT value, value_type, expires_at FROM cache_entries WHERE key = ?", (key,)
                    ) as cursor,
                ):
            ...
            try:
                async with aiosqlite.connect(self.db_path) as db:
                    ...
                    async with db.execute(
                        "SELECT COUNT(*) FROM cache_entries WHERE expires_at IS NOT NULL AND expires_at < ?", (now,)
                    ) as cursor:
            ...
            try:
                async with aiosqlite.connect(self.db_path) as db:
                    ...
                    if expired_keys:
                        ...
                        await db.execute(f"DELETE FROM cache_entries WHERE key IN ({placeholders})", expired_keys)
                        await db.commit()
...
def get_disk_cache(db_path: str = "cache/cache.db", default_ttl: int = 3600) -> DiskCache:
    """获取全局磁盘缓存实例"""
    global _global_disk_cache

    if _global_disk_cache is None:
        _global_disk_cache = DiskCache(db_path=db_path, default_ttl=default_ttl)

...
class EmbyNamingService:
    ...
    def generate_tv_show_name(
        ...
        if is_special:
            ...
            if episode:
                file_name = self.config.specials_episode_template.format(title=safe_title, episode=episode) + extension
            else:
            ...
            if episode is not None:
                if episode_title and self.config.tv_episode_with_title_template:
                    file_name = (
                        self.config.tv_episode_with_title_template.format(
                            title=safe_title,
                            season=season,
                            episode=episode,
                    ...
                    file_name = (
                        self.config.tv_episode_template.format(title=safe_title, season=season, episode=episode)
                        + extension
                    )
            else:
    ...
    def generate_subtitle_name(
        self, video_name: str, language: str, is_forced: bool = False, is_default: bool = False, is_sdh: bool = False
    ) -> str:
        """
        生成字幕文件名
    ...
    def parse_quality(self, filename: str) -> str | None:
        """
        从文件名解析质量信息
    ...
    def parse_source(self, filename: str) -> str | None:
        """
        从文件名解析来源信息
    ...
    def parse_release_group(self, filename: str) -> str | None:
        """
        从文件名解析发布组信息
    ...
    def validate_emby_naming(self, filename: str) -> dict[str, Any]:
        ...
        副作用: 无
        """
        result = {"is_valid": True, "suggestions": [], "warnings": []}
...
# 全局服务实例
_naming_service: EmbyNamingService | None = None


def get_emby_naming_service(config: NamingConfig | None = None) -> EmbyNamingService:
    """

...
from app.schemas.file_manager import BrowseResponse, FileOperationAction, FileOperationRequest, StorageType
...
class FileManagerService:
    ...
    def __init__(self):
        ...
        try:
            self._providers[StorageType.LOCAL] = LocalStorageProvider()
        except HTTPException as exc:
            logger.warning(f"Local storage provider unavailable: {exc.detail}")
    ...
    async def browse(
        self, storage_type: StorageType, path: str = "/", page: int = 1, size: int = 100
    ) -> BrowseResponse:
        """浏览目录"""
        ...
        return BrowseResponse(items=items, total=total, path=path, parent_path=parent_path)
    ...
    async def handle_operation(self, request: FileOperationRequest) -> dict:
        ...
        if request.action == FileOperationAction.MKDIR:
            if not request.target:

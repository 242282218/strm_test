...
from app.services.media.files import VIDEO_EXTENSIONS as MEDIA_VIDEO_EXTENSIONS
from app.services.media.files import is_video_path
from app.services.quark_api_client_v2 import QuarkAPIClient


logger = get_logger(__name__)

VIDEO_EXTENSIONS = MEDIA_VIDEO_EXTENSIONS
...
class QuarkService:
    ...
    async def get_files(self, parent: str, page_size: int = 100, only_video: bool = False) -> list[FileModel]:
        """
        获取文件列表
        ...
        while True:
            ...
            try:
                ...
                for file_data in file_list:
                    ...
                    normalized_file["file_name"] = unescape(normalized_file.get("file_name", ""))
                    file_model = self._file_model_from_info(normalized_file)

                    if only_video and (file_model.is_dir or file_model.category != 1):
                        continue
                ...
                logger.error(f"Failed to get files from {parent}: {e!s}")
                break
    ...
    async def get_transcoding_link(self, file_id: str) -> LinkModel:
        ...
        return LinkModel(url=transcoding_url, concurrency=3, part_size=10 * 1024 * 1024)
    ...
    async def list_files(self, pdir_fid: str = "0", page: int = 1, size: int = 100) -> dict[str, Any]:
        """
        获取文件列表（分页）
        ...
        try:
            ...
            return {"list": file_list, "metadata": metadata}

        except Exception as e:
            logger.error(f"List files failed for pdir_fid={pdir_fid}: {e!s}")
            raise
    ...
    async def rename_file(self, fid: str, new_name: str) -> dict[str, Any]:
        ...
        try:
            ...
            logger.error(f"Rename file failed for fid={fid}: {e!s}")
            raise

    async def move_file(self, fid: str, to_pdir_fid: str, new_name: str | None = None) -> dict[str, Any]:
        """
        移动文件到指定目录
        ...
        try:
            ...
            return {"fid": fid, "to_pdir_fid": to_pdir_fid, "status": "success"}

        except Exception as e:
            logger.error(f"Move file failed for fid={fid}: {e!s}")
            raise
    ...
    def is_video_file(self, file_name: str) -> bool:
        ...
        副作用: 无
        """
        return is_video_path(file_name)

    async def get_all_video_files(
        self, pdir_fid: str = "0", recursive: bool = True, max_files: int = 1000
    ) -> list[dict[str, Any]]:
        """
        递归获取所有视频文件
        ...
        async def scan_directory(dir_fid: str, depth: int = 0):
            ...
            try:
                ...
                logger.error(f"扫描目录失败 {dir_fid}: {e!s}")

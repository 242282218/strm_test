...
from app.services.media.rename import get_rename_service


logger = get_logger(__name__)
config_service = get_config_service()
ORGANIZE_VIDEO_EXTENSIONS = frozenset({".mp4", ".mkv", ".avi", ".mov", ".wmv", ".flv", ".ts", ".m2ts"})
...
class _RuntimeOrganizeConfigFacade:
    def get(self, key: str, default=None):
        app_config = config_service.get_config()

        if key == "quark.cookie":
            return str(getattr(getattr(app_config, "quark", None), "cookie", "") or "").strip()
        if key == "webdav.username":
            return str(getattr(getattr(app_config, "webdav", None), "username", "") or "").strip()
        if key == "webdav.password":
            return str(getattr(getattr(app_config, "webdav", None), "password", "") or "").strip()
        if key == "app.port":
            app_section = getattr(app_config, "app", None)
            fallback = 8000 if default is None else default
            return int(getattr(app_section, "port", fallback) or fallback)
        if key == "app.host":
            app_section = getattr(app_config, "app", None)
            fallback = "localhost" if default is None else default
            return str(getattr(app_section, "host", fallback) or fallback)
...
class MediaOrganizeService:
    def __init__(self, db_session):
        ...
        self.config = _RuntimeOrganizeConfigFacade()
            ...
            for i, file in enumerate(scanned_files):
                try:
                    ...
                    match_info, confidence = await self.rename_service._match_media(
                        file.file_name, media_type=media_type
                    )
    ...
    async def _scan_recursive(self, service: QuarkService, root_path: str) -> list[ScannedFile]:
        ...
        async def _walk(fid, current_path):
            ...
            for f in files:
                ...
                elif self._is_video(f.file_name):
                    results.append(ScannedFile(fid=f.fid, file_name=f.file_name, full_path=child_path, size=f.size))

        if root_file.is_dir:
            await _walk(root_file.fid, root_path)
        elif self._is_video(root_file.file_name):
            results.append(
                ScannedFile(fid=root_file.fid, file_name=root_file.file_name, full_path=root_path, size=root_file.size)
            )

        return results

    def _is_video(self, filename: str) -> bool:
        return is_video_path(filename, ORGANIZE_VIDEO_EXTENSIONS)

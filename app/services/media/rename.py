...
from app.services.media.files import RELATED_EXTENSIONS, build_stable_file_id, discover_media_files, find_related_files
from app.services.notification_service import NotificationType, get_notification_service
from app.services.tmdb_service import TMDBService, _global_tmdb_service, get_tmdb_service
...
class RenameService:
    ...
    def _get_tmdb_service(self) -> TMDBService | None:
    ...
    def _generate_file_id(self, path: str) -> str:
        """生成文件唯一ID"""
        return build_stable_file_id(path)

    async def _scan_media_files_async(self, path: str, recursive: bool = True) -> list[str]:
        """异步扫描目录获取媒体文件。"""
        return await asyncio.to_thread(discover_media_files, path, recursive)
    ...
    async def preview_rename(self, path: str, **kwargs) -> dict[str, Any]:
        ...
        """
        try:
            result = await self.preview(target_path=path, media_type=kwargs.get("media_type", "auto"), options=kwargs)
            return {
    ...
    async def preview(
        self, target_path: str, media_type: str = "auto", options: dict[str, Any] = None
    ) -> RenamePreviewResult:
        """预览重命名结果（不实际修改文件）"""
        ...
        async def process_file(file_path):
            ...
            item = RenameItem(
                original_path=file_path, original_name=filename, related_files=self._find_related_files(file_path)
            )
    ...
    async def execute(self, batch_id: str) -> RenameExecuteResult:
        ...
        try:
            ...
            items = (
                db.query(RenameHistory)
                .filter(RenameHistory.batch_id == batch_id, RenameHistory.status == "matched")
                .all()
            )
            ...
            await self.notification_service.send_notification(
                type=NotificationType.TASK_COMPLETED, title="媒体整理完成", content=f"成功: {success}, 失败: {failed}"
            )
    ...
    async def rollback(self, batch_id: str) -> RenameExecuteResult:
        ...
        try:
            ...
            items = (
                db.query(RenameHistory)
                .filter(RenameHistory.batch_id == batch_id, RenameHistory.status == "success")
                .order_by(RenameHistory.executed_at.desc())
                .all()
            )
            ...
            await self.notification_service.send_notification(
                type=NotificationType.TASK_COMPLETED, title="回滚完成", content=f"成功回滚: {success}, 失败: {failed}"
            )

            return RenameExecuteResult(
                batch_id=batch_id, total_items=len(items), success_items=success, failed_items=failed, skipped_items=0
            )

        finally:
            db.close()

    def get_batch(self, batch_id: str) -> RenameBatch | None:
        """获取批次详情"""
        db = SessionLocal()
        try:
    ...
    def list_batches(self, skip: int = 0, limit: int = 20) -> list[RenameBatch]:
        ...
        db = SessionLocal()
        try:
            return db.query(RenameBatch).order_by(RenameBatch.created_at.desc()).offset(skip).limit(limit).all()
        finally:
            db.close()
    ...
    def _find_related_files(self, file_path: str) -> list[str]:
        ...
        Returns:
            关联文件路径列表
        """
        return find_related_files(file_path)

    async def _match_media(
        self, filename: str, media_type: str = "auto", use_ai: bool = False
    ...
    def _calculate_confidence(self, parsed_info: dict[str, Any], match_info: dict[str, Any]) -> float:
        """
        计算匹配置信度
    ...
    def _generate_new_name(
        self, match_info: dict[str, Any], ext: str, options: dict[str, Any] | None = None
    ) -> tuple[str, str]:
        """
        生成新的文件名
        ...
        if media_type == "movie":
            # 电影命名
            if year:
                folder_name = self.TEMPLATES["movie_folder"].format(title=safe_title, year=year)
            ...
            # 电视剧命名 - 文件夹包含Season信息
            if year:
                show_folder = self.TEMPLATES["tv_show_folder"].format(title=safe_title, year=year)
            else:
            ...
            if season is not None and episode is not None:
                if episode_title:
                    file_name = (
                        self.TEMPLATES["tv_episode_with_title"].format(
                            title=safe_title, season=season, episode=episode, episode_title=episode_title
                        )
                        + ext
                    )
                else:
                    file_name = (
                        self.TEMPLATES["tv_episode"].format(title=safe_title, season=season, episode=episode) + ext
                    )
            else:
                file_name = f"{safe_title}{ext}"

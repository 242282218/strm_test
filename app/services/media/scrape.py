...
from app.services.media.files import discover_media_files
from app.services.media.scrape_state_machine import ScrapeStateMachine
...
SCRAPE_MEDIA_EXTENSIONS = frozenset({".mp4", ".mkv", ".avi", ".mov", ".strm"})
...
class ScrapeService:
    ...
    def _parse_error_payload(self, raw_error: str | None) -> tuple[str | None, str | None]:
        if not raw_error:
            return None, None
        try:
    ...
    def _build_allowed_dirs(self, options: dict[str, Any], *extra_paths: str | None) -> list[str]:
        allowed_dirs: list[str] = []

        def _append_dir(path: str | None) -> None:
    ...
    def _scan_directory(self, path: str) -> list[str]:
        return discover_media_files(path, extensions=SCRAPE_MEDIA_EXTENSIONS)
    ...
    def _build_simple_nfo(self, item: ScrapeItem) -> str:
        ...
        if item.media_type == "tv":
            ...
            episode_tag = (
                f"<episode>{self._xml_escape(str(item.episode))}</episode>" if item.episode is not None else ""
            )
            return (
    ...
    def _organize_item(
        ...
        try:
            ...
            return (
                False,
                None,
                self._standardize_error(
                    "PATH_SECURITY_ERROR", "Target directory is outside of allowed directories", str(e)
                ),
            )
    ...
    def _standardize_error(self, code: str, message: str, detail: str | None = None) -> dict[str, str]:

...
class SmartRenameService:
    ...
    async def _parse_with_algorithm(
        self, filename: str, algorithm: AlgorithmType, force_ai: bool = False, ai_timeout_seconds: int | None = None
        ...
        if algorithm == AlgorithmType.AI_ENHANCED or force_ai:
            ...
            parsed_info = await MediaParser.parse_with_ai(
                filename, force=force_ai, ai_timeout_seconds=ai_timeout_seconds
            )
    ...
    async def _match_tmdb(
        self, parsed_info: dict[str, Any], media_type_hint: str | None = None
    ...
    def _calculate_match_confidence(
        self, parsed_info: dict[str, Any], match_title: str, match_date: str | None
    ) -> float:
        """计算 TMDB 匹配置信度"""
        confidence = 0.0
    ...
    def _generate_new_name(self, item: SmartRenameItem, options: SmartRenameOptions) -> tuple[str, str]:
        """
        生成新的文件名和路径
        ...
        if media_type == "movie":
            ...
            if options.create_folders and season_folder:
                new_path = os.path.join(os.path.dirname(item.original_path), show_folder, season_folder, file_name)
            elif options.create_folders:
                new_path = os.path.join(os.path.dirname(item.original_path), show_folder, file_name)
            else:
                new_path = os.path.join(os.path.dirname(item.original_path), file_name)
    ...
    async def preview(self, target_path: str, options: SmartRenameOptions | None = None) -> SmartRenamePreviewResult:
        """
        预览重命名结果
        ...
        async def process_file(file_path: str) -> SmartRenameItem:
            ...
            parsed_info, algorithm, parse_confidence = await self._parse_with_algorithm(
                filename, options.algorithm, force_ai=options.force_ai_parse
            )
            ...
            # 匹配 TMDB
            tmdb_match, match_confidence = await self._match_tmdb(parsed_info, media_type_hint=item.media_type)
    ...
    async def execute(self, batch_id: str, operations: list[dict[str, str]] | None = None) -> dict[str, Any]:
        """
        执行重命名操作
        ...
        try:
            ...
            if operations:
                ...
                if candidate_paths:
                    ...
                    for item in items:
                        ...
                        if override_name:
                            target_dir = (
                                os.path.dirname(item.new_path) if item.new_path else os.path.dirname(item.original_path)
                            )
                ...
                items = (
                    db.query(RenameHistory)
                    .filter(RenameHistory.batch_id == batch_id, RenameHistory.status.in_(["matched", "parsed"]))
                    .all()
                )
            ...
            await self.notification_service.send_notification(
                type=NotificationType.TASK_COMPLETED, title="智能重命名完成", content=f"成功: {success}, 失败: {failed}"
            )
    ...
    async def rollback(self, batch_id: str) -> dict[str, Any]:
        ...
        try:
            ...
            return {"batch_id": batch_id, "total_items": len(items), "success_items": success, "failed_items": failed}

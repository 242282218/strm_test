...
from app.utils.strm_url import build_stable_media_url
...
class _RuntimeStrmConfigFacade:
    @staticmethod
    def get_webdav_config() -> dict[str, Any]:
        app_config = config_service.get_config()
        webdav_cfg = getattr(app_config, "webdav", None)
        if webdav_cfg is None:
            return {}
        if hasattr(webdav_cfg, "model_dump"):
            return webdav_cfg.model_dump()
        if isinstance(webdav_cfg, dict):
            return dict(webdav_cfg)
        return dict(vars(webdav_cfg))
    ...
    @staticmethod
    def get_quark_root_id() -> str:
        app_config = config_service.get_config()
        value = getattr(getattr(app_config, "quark", None), "root_id", "0")
        return str(value or "0").strip() or "0"

    @staticmethod
    def get_quark_only_video() -> bool:
        app_config = config_service.get_config()
        return bool(getattr(getattr(app_config, "quark", None), "only_video", True))


def get_config() -> _RuntimeStrmConfigFacade:
    return _RuntimeStrmConfigFacade()
...
class STRMGenerator:
        ...
        try:
            ...
            for file_info, result in zip(all_files, results):
                if isinstance(result, Exception):
                    ...
                    error_msg = f"Failed to generate STRM for {file_info.get('name', 'unknown')}: {result!s}"
            ...
            logger.error(f"Failed to generate STRM files: {e!s}")
    ...
    async def _get_all_files(
        ...
        try:
            ...
            logger.error(f"Failed to get files from {parent_id}: {e!s}")

        return files

    async def _generate_single_strm(self, file_info: dict[str, Any]) -> str | None:
        """
        生成单个STRM文件
        ...
        try:
            ...
            logger.error(f"Failed to generate STRM for {file_name}: {e!s}")
            raise

    async def generate_single_file_strm(self, file_id: str, remote_path: str) -> str | None:
        """
        为单个文件生成 STRM。
    ...
    async def _generate_video_url(self, file_id: str, remote_path: str = None) -> str:
        ...
        if self.strm_url_mode == "redirect":
            normalized_remote_path = (remote_path or file_id).replace("\\", "/").lstrip("/")
            mapping = self.media_mapping_service.get_or_create(
                provider_file_id=file_id,
                source_path=normalized_remote_path,
                display_name=Path(normalized_remote_path).name or file_id,
            )
            return build_stable_media_url(self.base_url, mapping.media_id, mapping.display_name)

        if self.strm_url_mode == "stream":
            # 流代理模式
            # Emby请求此URL时，后端会代理视频流数据（占用服务器带宽）
            return f"{self.base_url}/api/proxy/stream/{file_id}"

        if self.strm_url_mode == "direct":
            # 直链模式（不推荐，链接会过期）
...
async def generate_strm_from_quark(
    ...
    generator = STRMGenerator(cookie=cookie, output_dir=output_dir, base_url=base_url, strm_url_mode=strm_url_mode)

    try:
        result = await generator.generate_strm_files(root_id=root_id, only_video=only_video, max_files=max_files)
        return result
    finally:

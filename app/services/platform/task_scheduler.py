...
class Task:
    ...
    async def run(self, handler: Callable):
        ...
        try:
            ...
            logger.error(f"Task {self.task_id} failed: {e!s}")
        finally:
            # 计算下次运行时间
            self._calculate_next_run()
...
class TaskScheduler:
    ...
    def add_task(
        self, name: str, mode: TaskMode, interval_type: str, interval_value: int, config_id: str, enabled: bool = True
    ) -> Task:
        """
        添加任务
    ...
    def get_status(self) -> dict[str, Any]:
        ...
        Returns:
            状态字典
        """
        return {"running": self._running, "task_count": len(self.tasks), "tasks": self.list_tasks()}

...
统一缓存接口说明:
- 本模块实现 TieredCacheProtocol 接口
- 可作为 CacheManager 的可选后端使用
- 推荐通过 CacheManager.get_backend_adapter(BackendType.TIERED) 使用

使用示例:
    from app.core.cache_manager import get_cache_manager, BackendType

    cache = get_cache_manager()
    adapter = cache.get_backend_adapter(BackendType.TIERED)
...
class TieredCache:
    ...
    def __init__(self, config: CacheConfig = None):
        ...
        self._l2_cache: DiskCache | None = None
        self._l3_cache: Any | None = None
        ...
        logger.info(
            f"TieredCache initialized: L1={self.config.enable_l1}, L2={self.config.enable_l2}, L3={self.config.enable_l3}"
        )

    def _init_caches(self):
        """初始化各级缓存实例"""
        if self.config.enable_l1:
            self._l1_cache = MemoryCache(max_size=self.config.l1_max_size, default_ttl=self.config.l1_default_ttl)

        if self.config.enable_l2:
            self._l2_cache = DiskCache(db_path=self.config.l2_db_path, default_ttl=self.config.l2_default_ttl)
        ...
        if self.config.enable_l3:
            try:
                ...
                self._l3_cache = get_redis_cache_backend(
                    redis_url=self.config.l3_redis_url, default_ttl=self.config.l3_default_ttl
                )
    ...
    async def set(self, key: str, value: Any, ttl: int | None = None, persist: bool = True) -> bool:
        """
        设置缓存值
    ...
    async def get_or_set(self, key: str, factory: Callable, ttl: int | None = None, persist: bool = True) -> Any:
        """
...
# 全局实例
_global_tiered_cache: TieredCache | None = None

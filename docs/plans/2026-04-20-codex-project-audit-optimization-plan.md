...
| Phase 3 | 部分完成 | `config/db/exception` 边界文档、`resolve_db_path()` contract、app 层 compatibility-caller 清理、TMDB / stable-stream API 运行态 caller 收口与 import guard 已补，但基础设施代码尚未进入实质拆分。 |
...
| Iteration 3 | 已完成到边界文档 / caller 收敛层 | 已完成 `config/db/exception` 入口边界文档化、STRM API 的无效 `Database` 兼容层实例化清理，以及 TMDB / stable-stream API 的配置兼容 caller 收口；尚未进入基础设施重构。 |

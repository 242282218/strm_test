...
- `app/services/token_monitor.py` 与 `app/services/webdav_fallback.py` 已移除 `get_config()` compatibility import；Quark cookie 与 WebDAV fallback 配置统一通过 `get_config_service()` 读取，同时保留最小 helper 作为测试 patch 点，并由对应 service test + `tests/test_db_path_contract.py` 锁定。
- service/core 层仍保留 11 个 `config_manager` compatibility caller：`app/core/path_security.py`、`app/services/ai_connectivity_service.py`、`app/services/emby_proxy_service.py`、`app/services/integrations/emby.py`、`app/services/link_resolver.py`、`app/services/media/organize.py`、`app/services/media/rename.py`、`app/services/media/smart_rename.py`、`app/services/media/strm_generator.py`、`app/services/storage/quark.py`、`app/services/unified_ai_service.py`；当前新增的是 inventory contract，尚未进入逐模块清理。

...
- `app/services/token_monitor.py`、`app/services/webdav_fallback.py` 与 `app/core/path_security.py` 已移除 `config_manager` compatibility import；Quark cookie、WebDAV fallback 配置与允许目录补充读取统一通过 `get_config_service()` 读取，同时保留最小 helper 作为测试 patch 点，并由对应模块测试 + `tests/test_db_path_contract.py` 锁定。
- service/core 层仍保留 10 个 `config_manager` compatibility caller：`app/services/ai_connectivity_service.py`、`app/services/emby_proxy_service.py`、`app/services/integrations/emby.py`、`app/services/link_resolver.py`、`app/services/media/organize.py`、`app/services/media/rename.py`、`app/services/media/smart_rename.py`、`app/services/media/strm_generator.py`、`app/services/storage/quark.py`、`app/services/unified_ai_service.py`；当前新增的是 inventory contract，尚未进入逐模块清理。

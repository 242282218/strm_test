    ...
    def test_legacy_prefixes_detection(self):
        ...
        for path, expected_is_legacy in test_cases:
            is_legacy = (
                ...
                and any(path.startswith(prefix) for prefix in legacy_prefixes)
            )
            assert is_legacy == expected_is_legacy, f"Path {path}: expected {expected_is_legacy}, got {is_legacy}"
...
class TestMainRouterRegistration:
    ...
    def test_main_app_prefers_canonical_v1_tasks_collection_over_legacy_dynamic_alias(

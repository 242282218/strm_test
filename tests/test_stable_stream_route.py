...
def test_stable_stream_route_when_native_authorization_header_marks_force_proxy_client_then_redirects_to_local_proxy_stream():
    app = FastAPI()
    app.include_router(router)
    client = TestClient(app)
    ...
    with (
        ...
        mock_get_config_service.return_value.get_config.return_value = mock_config
        response = client.get(
            "/strm/v1/m/media123/Avatar%20(2009).mkv",
            headers={"Authorization": 'Emby Client="Infuse", Device="Apple TV"'},

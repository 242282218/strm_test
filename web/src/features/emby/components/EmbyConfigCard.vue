<template>
  <el-card class="emby-config-card">
    ...
    <el-form :model="form" label-width="140px" :disabled="loading">
      ...
      <el-form-item label="代理基址">
        <el-input
          v-model="form.proxy_base_url"
          placeholder="http://localhost:18097"
          :disabled="!form.enabled"
        />
        <div class="form-tip">Emby 播放时访问 smart_media 代理的地址，建议使用独立入口端口</div>
      </el-form-item>
...
const saveConfig = async () => {
  ...
  try {
    ...
    ElMessage.error(getErrorDetail(error, '保存失败'))
  } finally {
    saving.value = false
  }
}
...
const manualRefresh = async () => {
  try {
    ...
    ElMessage.error(getErrorDetail(error, '刷新失败'))
  }
}
...
onMounted(async () => {
  ...
  try {
    ...
    form.proxy_base_url = status.configuration.proxy_base_url || ''

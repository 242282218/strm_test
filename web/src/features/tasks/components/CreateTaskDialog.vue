...
import { createTask, type TaskCreateRequest } from '@/features/tasks/api/tasks'

const props = withDefaults(defineProps<{
  modelValue: boolean
  initialTaskType?: TaskCreateRequest['task_type']
}>(), {
  initialTaskType: 'strm_generation'
})
...
const createDefaultParams = (taskType: TaskCreateRequest['task_type']): TaskCreateRequest['params'] => {
  switch (taskType) {
    case 'file_sync':
      return {
        remote_path: '/video',
...
const form = reactive<TaskCreateRequest>({
  task_type: props.initialTaskType,
  priority: 'normal',
  params: createDefaultParams(props.initialTaskType)
})
...
const applyTaskType = (taskType: TaskCreateRequest['task_type']) => {
  form.task_type = taskType
  form.params = createDefaultParams(taskType)
}

const resetFormState = () => {
  applyTaskType(props.initialTaskType)
  form.priority = 'normal'
}

const onTaskTypeChange = () => {
  applyTaskType(form.task_type)
}
...
watch(() => props.initialTaskType, (taskType) => {
  if (!visible.value || taskType === form.task_type) return

  applyTaskType(taskType)
  formRef.value?.clearValidate()
})
</script>
